                                ...
                                <tr class="border-b border-gray-200 hover:bg-gray-100">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">                             
                                        <div class="flex space-x-2">
                                            <a href="{{ route('pdf.compra',$compra->nro) }}" class="bg-green-500 hover:bg-green-700 text-white font-bold py-1 px-3 rounded-lg transition duration-300">
                                                generar PDF
                                            </a>                        
                                            <a href="{{ route('compra.edit', $compra->nro) }}" class="bg-yellow-500 hover:bg-yellow-600 text-white font-bold py-1 px-3 rounded-lg transition duration-300">
                                                Editar Compra
                                            </a>
                                            <form action="{{ route('compra.delete', $compra->nro) }}" method="POST" onsubmit="return confirm('¿Estás seguro de que deseas eliminar este producto?');">
                                                @csrf
                                                @method('DELETE')
